...
const ConnectedScatter = {
  render: function(container, data, opts = {}) {
    ...
    const colorScale = d3.scaleSequential()
      ...
      .interpolator(d3.interpolateRgb(CONFIG.colors.accent2, CONFIG.colors.accent));
    ...
    if (opts.xLabel) {
      svg.append('text')
        ...
        .attr('font-size', '0.7rem').attr('fill', CONFIG.colors.caption)
        .text(opts.xLabel);
    }
    if (opts.yLabel) {
      svg.append('text')
    ...
    svg.selectAll('.scatter-dot')
      ...
      .on('mousemove', function(event, d) {
        d3.select(this).attr('r', 7).attr('stroke', CONFIG.colors.accent).attr('stroke-width', 2);
        Tooltip.show(event, `
      ...
      .on('mouseleave', function() {
        d3.select(this).attr('r', 4).attr('stroke', CONFIG.colors.bg).attr('stroke-width', 1);
        Tooltip.hide();
      });
