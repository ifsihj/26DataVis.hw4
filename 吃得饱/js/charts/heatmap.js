...
const Heatmap = {
  render: function(container, data, opts = {}) {
    ...
    const colorScheme = opts.colorScheme || ['#F2EDE4', '#D4C9C3', '#C4A4A4', '#9E6B6B', '#7D4B4B'];
    ...
    lg.append('text').attr('x', -4).attr('y', legendH / 2 + 3)
      .attr('text-anchor', 'end').attr('font-size', '0.6rem').attr('fill', '#888')
      .text(vExtent[0]);
    lg.append('text').attr('x', legendW + 4).attr('y', legendH / 2 + 3)
      .attr('text-anchor', 'start').attr('font-size', '0.6rem').attr('fill', '#888')
      .text(vExtent[1]);
